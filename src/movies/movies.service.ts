...
export class MoviesService {
  ...
  async searchMovies(searchMoviesDto: SearchMoviesDto): Promise<any> {
    const { search, page } = searchMoviesDto;
    let result: any;
 
    try {
      result = await this.httpService.get(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=en-US&query=${search}&page=${page}&include_adult=false`).toPromise();
      return result.data;
    } catch (error) {
